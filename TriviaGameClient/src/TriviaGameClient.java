...
public class TriviaGameClient extends JFrame implements Runnable
{
	...
	public void run()
	{
		...
		output_stream.println("score:"+screenname+"-"+score);
		...
		try
		{
			while((response = input_stream.readLine()) != null)
			{
				...
				if(response.startsWith("scoreboard:"))
				{
					//System.out.println(Integer.parseInt(response.substring(6)));
					//output_stream.println("score:"+screenname+"-"+response.substring(6));
	...
	private class WindowClose implements WindowListener
	{
		...
		public void windowClosing(WindowEvent arg0) {
			output_stream.println("early_quit");
		}
