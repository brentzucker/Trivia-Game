...
public class TriviaGameClientThread extends Thread
{
	// Load Server configuration
		...
		public PrintStream output_stream = null;
		...
		try
		{
			...
			for(TriviaGameClientThread thread: TriviaGameServer.running_threads){
				if(thread == this){
					...
					//decrement player count if a player leaves
					TriviaGameServer.player_count--;
					
					//reset game when game ends - game ends when there are no running threads 
					if(TriviaGameServer.running_threads.size() == 0)
					{
						TriviaGameServer.resetGame();
					}
